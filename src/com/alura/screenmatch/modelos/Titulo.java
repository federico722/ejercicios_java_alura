...
public class Titulo implements Comparable<Titulo> {
    ...
    public Titulo(TituloOmdb miTitulo0mdb) {
        this.nombre = miTitulo0mdb.Title();
        this.fechaDeLanzamiento = Integer.valueOf(miTitulo0mdb.Year());
        if (miTitulo0mdb.Runtime().contains("N/A")){
            throw new ErrorEnConversionDeDuracionException("No pude convertir la duracion." +
                    "porque contiene un N/A");
        }
        this.duracionEnMinutos = Integer.valueOf(miTitulo0mdb.Runtime().substring(0,3).replace(" ", ""));
    }
